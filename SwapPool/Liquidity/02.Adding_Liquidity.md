...
This guide will cover how to modify a liquidity position by adding liquidity in ICPSwap. Before starting, try to get the id of ICP - ckBTC SwapPool canister by [Searching a Pool](../../SwapFactory/01.Searching_a_Pool.md). In the returned data, it can be determined what tokens token0 and token1 are in the current SwapPool.
...
If the increaseLiquidity method call fails, the deposited token balance will still be recorded in that user's account in SwapPool, which can be queried through the **getUserUnusedBalance** method, with the parameter passed in as the user principal. In this case, the user can use the **withdraw** method to extract the token, or recall the **increaseLiquidity** method again based on the result of **getUserUnusedBalance**.